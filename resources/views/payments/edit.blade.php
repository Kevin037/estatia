<x-admin-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="card">
                <form action="{{ route('payments.update', $payment->id) }}" method="POST" id="paymentForm">
                    ...
                    <div class="form-group">
                        <label for="payment_no" class="form-label">Payment Number</label>
                        <input type="text" id="payment_no" value="{{ $payment->no }}" class="form-input bg-gray-50" readonly>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="dt" id="dt" value="{{ old('dt', $payment->dt?->format('Y-m-d')) }}" class="form-input" required>
                        @error('dt')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="payment_type" id="payment_type" class="form-select" required>
                            ...
                            <option value="transfer" {{ old('payment_type', $payment->payment_type) == 'transfer' ? 'selected' : '' }}>Bank Transfer</option>
                        </select>
                        @error('payment_type')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="amount" id="amount" value="{{ old('amount', $payment->amount) }}" class="form-input" step="0.01" min="0" required>
                        @error('amount')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div id="bankFields" style="display: {{ old('payment_type', $payment->payment_type) == 'transfer' ? 'block' : 'none' }};">
                        <div class="form-group">
                            <label for="bank_account_id" class="form-label required">Account Number</label>
                            <input type="text" name="bank_account_id" id="bank_account_id" value="{{ old('bank_account_id', $payment->bank_account_id) }}" class="form-input">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="bank_account_type" id="bank_account_type" value="{{ old('bank_account_type', $payment->bank_account_type) }}" class="form-input" placeholder="e.g., BCA, Mandiri, BNI">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="bank_account_name" id="bank_account_name" value="{{ old('bank_account_name', $payment->bank_account_name) }}" class="form-input">
        ...
        <div class="lg:col-span-1">
            <div class="card sticky top-6" id="invoicePreview">
    ...
    <script>
        $(document).ready(function() {
            ...
            // Load invoice preview on page load with current payment's invoice
            @if($payment->invoice_id)
                loadInvoicePreview({{ $payment->invoice_id }});
            @endif
