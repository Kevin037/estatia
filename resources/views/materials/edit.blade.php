        ...
        <form action="{{ route('materials.update', $material) }}" method="POST">
            ...
            <div class="card mb-6">
                <div class="card-header">
                    ...
                    <p class="mt-1 text-sm text-gray-600">Update material information below</p>
                </div>

                <div class="grid grid-cols-1 gap-6">
                    <div>
                        <label for="name" class="form-label">Name <span class="text-red-500">*</span></label>
                        <input type="text" name="name" id="name" value="{{ old('name', $material->name) }}" class="form-input @error('name') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Material name" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price', $material->price) }}" step="0.01" min="0" class="form-input @error('price') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="0.00" required>
                        @error('price')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="px-3 py-2 bg-gray-50 border border-gray-300 rounded-md text-gray-900">
                            {{ $material->supplier ? $material->supplier->name : 'No supplier assigned' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between bg-gray-50 px-6 py-4 rounded-lg">
                ...
                <button 
                    ...
                    <span x-text="loading ? 'Updating…' : 'Update Material'"></span>
                </button>
            </div>
        </form>
    </div>
