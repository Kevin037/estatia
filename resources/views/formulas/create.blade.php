<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-gray-900">Add Formula</h2>
    ...
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('formulas.store') }}" method="POST" x-data="formulaForm()">
            @csrf

            <div class="card mb-6">
                <div class="card-header">
                    <h3 class="text-lg font-semibold text-gray-900">Formula Information</h3>
                    <p class="mt-1 text-sm text-gray-600">Add a new formula with material details</p>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="code" class="form-label">Formula Code <span class="text-red-500">*</span></label>
                        <input type="text" name="code" id="code" value="{{ old('code') }}" class="form-input @error('code') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="e.g., F-001" required>
                        @error('code')
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-input @error('name') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" placeholder="Formula name" required>
                        @error('name')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="card mb-6">
                <div class="card-header flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600">Add materials and quantities for this formula</p>
                    </div>
            ...
            <div class="flex items-center justify-between bg-gray-50 px-6 py-4 rounded-lg">
                ...
                <button 
                    ...
                    <span x-text="loading ? 'Creating…' : 'Create Formula'"></span>
                </button>
            </div>
        </form>
    </div>
